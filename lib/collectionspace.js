...
CollectionSpace.prototype.getRecord = function(type, csid) {
  ...
  if (!this._connectionStatus || !this._connectionStatus.login) {
    ...
    this._agent
      .get(this._baseUrl + RECORD_READ_PATH + '/' + type + '/' + csid)
      .end(function(error, response) {
        if (error) {
          deferred.reject(error);
        }
...
CollectionSpace.prototype.createRecord = function(type, data) {
  var deferred = Q.defer();
  ...
  if (!this._connectionStatus || !this._connectionStatus.login) {
    ...
    this._agent
      .post(this._baseUrl + RECORD_CREATE_PATH + '/' + type + '/')
      .send(data)
      .end(function(error, response) {
        if (error) {
          deferred.reject(error);
...
CollectionSpace.prototype.updateRecord = function(type, csid, data) {
  var deferred = Q.defer();
  ...
  if (!this._connectionStatus || !this._connectionStatus.login) {
    ...
    this._agent
      .put(this._baseUrl + RECORD_UPDATE_PATH + '/' + type + '/' + csid)
      .send(data)
      .end(function(error, response) {
        if (error) {
          deferred.reject(error);
